...
char *word_start (char *str){
  ...
  while(space_char(*tmp)){ /*Iter through chars until a letter is hit*/
    tmp++;
  }
  return tmp;
}
...
char *word_terminator (char *word){
  ...
  while(non_space_char(*tmp)){ /*Iter chars until a space or null is hit*/
    tmp++;
  }
  return tmp;
}
...
char *copy_str(char *inStr, short len){
  ...
  str_copy[len] = '\0'; /*Places the null terminator at the end*/
  return str_copy;
}
